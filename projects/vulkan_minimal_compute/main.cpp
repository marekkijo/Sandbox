...
#include "lodepng.hpp" //Used for png encoding.

const int WIDTH          = 3200 * 4; // Size of rendered mandelbrot set.
const int HEIGHT         = 2400 * 4; // Size of renderered mandelbrot set.
...
public:
  ...
  void saveRenderedImage() {
    ...
    unsigned error = lodepng::encode("C:/Users/marek/Desktop/mandelbrot.png", image, WIDTH, HEIGHT);
  ...
  void createInstance() {
    ...
    */
    if (enableValidationLayers) {
      ...
      */
      ...
      if (!foundLayer) { throw std::runtime_error("Layer VK_LAYER_KHRONOS_validation not supported\n"); }
    ...
    */
    ...
    /*
    Register a callback function for the extension VK_EXT_DEBUG_REPORT_EXTENSION_NAME, so that warnings emitted from the
  ...
  void findPhysicalDevice() {
    ...
    */
    ...
    if (deviceCount == 0) { throw std::runtime_error("could not find a device with vulkan support"); }
    ...
    and the maximum size of the workgroups and total number of compute shader invocations is limited by the physical
    device, and we should ensure that the limitations named maxComputeWorkGroupCount, maxComputeWorkGroupInvocations and
    maxComputeWorkGroupSize are not exceeded by our application.  Moreover, we are using a storage buffer in the compute
    shader, and we should ensure that it is not larger than the device can handle, by checking the limitation
    maxStorageBufferRange.

    However, in our application, the workgroup size and total number of shader invocations is relatively small, and the
    storage buffer is not that large, and thus a vast majority of devices will be able to handle it. This can be
    ...
    physical device in the list. But in a real and serious application, those limitations should certainly be taken into
    account.
  ...
  uint32_t *readFile(uint32_t &length, const char *filename) {
    ...
    // data padding.
    for (int i = filesize; i < filesizepadded; i++) { str[i] = 0; }
  ...
  void createComputePipeline() {
    ...
    */
    ...
    uint32_t                *code       = readFile(filelength, "projects/vulkan_minimal_compute/shaders/comp.spv");
  ...
  void createCommandBuffer() {
    ...
    */
    ...
    VK_CHECK_RESULT(
        vkAllocateCommandBuffers(device, &commandBufferAllocateInfo, &commandBuffer)); // allocate command buffer.
  ...
  void cleanup() {
    ...
    */
    ...
    if (enableValidationLayers) {
      ...
      if (func == nullptr) { throw std::runtime_error("Could not load vkDestroyDebugReportCallbackEXT"); }
